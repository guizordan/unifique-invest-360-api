function isHttpErrorCode(httpCode) {
...
export function handleSend(req, reply, payload, done) {
    ...
    if (typeof message === "string" && message) {
        const messages = message.replace(",\n", "").split("Validation error: ").filter((item)=>item.trim() !== "");
...
export function handleError(ex, req, reply) {
    ...
    const errorResponse = {
        message: errors[0]?.message || message || "Ocorreu um erro inesperado. Por favor, tente novamente."
    };
