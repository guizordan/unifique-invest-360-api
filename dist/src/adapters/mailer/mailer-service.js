...
export function createEmailParams({ recipient, subject, htmlContent }) {
    const sentFrom = new Sender(ADMIN_SENDER, "unifique");
    const recipients = [
        new Recipient(recipient)
    ];
    return new EmailParams().setFrom(sentFrom).setTo(recipients).setReplyTo(sentFrom).setSubject(subject).setHtml(htmlContent);
}
export async function sendOrLogEmail(emailParams) {
    if (NODE_ENV === "prod") {
        try {
...
export function replacePlaceholders(emailTemplateName, replacements) {
    const emailTemplatePath = path.join(__dirname, "..", "assets", `${emailTemplateName}.html`);
